...
describe('MemoryStore (crypto)', function (done) {
  afterEach(function () {
  ...
  it('should set options', function (done) {
    ...
    assert.equal(store.secret, 'squirrel', 'should set secret')
    done()
  })

  it('should not set the interval to check for expired entries by default', function (done) {
    this.store = new session.MemoryStore({secret: "squirrel"})
    var store = this.store
  ...
  it('should only contain 10 items', function (done) {
    this.store = new session.MemoryStore({secret: "squirrel", max: 10})
    var store = this.store
  ...
  it('should enable automatic prune for expired entries', function (done) {
    this.store = new session.MemoryStore({secret: "squirrel", checkPeriod: 300})
    var store = this.store
